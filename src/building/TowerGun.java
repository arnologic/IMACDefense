...
public class TowerGun extends BuildingAbstract {
	...
	public TowerGun(int x, int y, int s, int id, int t){
		size = s;
		positionX = x;
		positionY = y;
		ID = id;
	...
	public TowerGun(int s, int id, int x, int y){
		size = 10;
		positionX = x;
		positionY = y;	
		ID = id;
    ...
    public void paintTower(Graphics2D g){
		...
		g.drawString("G", getPositionX()+(getSize()/2)-8, getPositionY()+(getSize()/2)+7);
    }
