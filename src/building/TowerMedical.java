...
public class TowerMedical extends BuildingAbstract {
	...
	public TowerMedical(int x, int y, int s, int id, int t){
		size = s;
		positionX = x;
		positionY = y;
		ID = id;
	...
	public TowerMedical(int s, int id, int x, int y){
		size = 10;
		positionX = x;
		positionY = y;	
		ID = id;
    ...
    public void paintTower(Graphics2D g){
		...
		g.drawString("M", getPositionX()+(getSize()/2)-11, getPositionY()+(getSize()/2)+7);
    }
