...
public class TowerLaser extends BuildingAbstract {
	...
	public TowerLaser(int x, int y, int s, int id, int t){
		size = s;
		positionX = x;
		positionY = y;
		ID = id;
	...
	public TowerLaser(int s, int id, int x, int y){
		size = 10;
		positionX = x;
		positionY = y;	
		ID = id;
    ...
    public void paintTower(Graphics2D g){
		...
		g.drawString("L", getPositionX()+(getSize()/2)-8, getPositionY()+(getSize()/2)+7);
    }
